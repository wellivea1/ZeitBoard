...
func TestProposalApprovalMaterializesOwnedBlockAndUndoRemovesOnlyIt(t *testing.T) {
	...
	input := ProposalDecisionInput{
		...
		ProposalTitle:     "Write planning review",
		ProposalStartAt:   time.Date(2026, 1, 6, 15, 0, 0, 0, time.UTC),
		ProposalEndAt:     time.Date(2026, 1, 6, 16, 0, 0, 0, time.UTC),
		ZoneID:            "America/New_York",
		Confidence:        "medium",
		ExplanationCodes:  []string{"within_task_bounds", "avoids_fixed_event"},
	...
	owned := calendarcore.Event{
		...
		Title:          "Write planning review",
		StartAt:        input.ProposalStartAt,
		EndAt:          input.ProposalEndAt,
		ZoneID:         "America/New_York",
		Busy:           true,
...
func TestProposalDecisionRejectsTaskAndCalendarRaces(t *testing.T) {
	...
	input := ProposalDecisionInput{
		...
		EstimateID:        "estimate_calendar_01",
		ProposalTitle:     "Write planning review",
		ProposalStartAt:   start.Add(20 * time.Hour),
		ProposalEndAt:     start.Add(21 * time.Hour),
		ZoneID:            "UTC",
		Confidence:        "medium",
		ExplanationCodes:  []string{"within_task_bounds"},
