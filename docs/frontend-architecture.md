...
- `src/data/` is the only UI layer that knows how to locate Wails methods. Each
  ...
  export counts, and the no-fixture invariant. The clinician-report adapter
  additionally reconciles date order, row and adherence totals, exact-present
  legend kinds, drift bounds, mandatory redactions, canonical filenames, and
  the offline HTML policy before anything renders or downloads.
...
clinician text; it never turns forecast context into a warning bubble. The
collapsed clinician-report section expands into ruled controls and preview
rather than cards. Its paged clinical-day chart owns narrow-width scrolling,
exposes semantic tables, marks changed controls as stale, and disables export
until the exact preview settings have been regenerated.
...
- Medication edits and exclusion append corrections to immutable raw events.
  ...
  forecast labels never move a time. M-A through M-C have no browser fixture
  fallback, sync projection, assistant context, or interaction check. M-C
  report HTML is generated by Go only after typed confirmation; labels and
  notes are opt-in, mandatory redactions are visible, and no network fallback
  exists.
