...
| 7 | §5.1/§5.4 — Home as a quiet instrument with quick actions | **Delivered 2026-08-09.** Home carries `I am going to sleep` and `I woke up` directly under the current state. The taps are not treated as observations: the first parks an intent, and the night is appended only when the pair is between 3 and 14 hours and under 20 hours old. Anything else returns a typed question — nap or mistap, missed wake tap, an onset too old for "now" to mean anything, or nothing marked at all — and records nothing until it is answered. A prefill taken from the forecast is labelled as a prediction on the field itself. Decision logic lives in `core/quicklog` so a companion surface answers identically. |
